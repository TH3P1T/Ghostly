               ...
               <div class="col-md-8">
                  ...
                  <div class="card card-default">
                     ...
                     <?php if ($add_attempted) { ?>
                        ...
                        <?php } ?>
                        <?php if (!$added) { ?>
                           <div class="alert alert-danger"><strong>Oh snap!</strong> Unable to add the new license to the database.</div>
                        <?php } ?>
                        <?php if ($added) { ?>
                           <div class="alert alert-success"><strong>Success!</strong> License added</div>
                        <?php } ?>
                     <?php } ?>
                     </div>
                     <div class="card-body">
                        <form method="post" action="<?= $url_base ?>licenses/add">
                           <input type="hidden" name="<?= $csrf_name_key ?>" value="<?= $csrf_name ?>" />
                           <input type="hidden" name="<?= $csrf_value_key ?>" value="<?= $csrf_value ?>" />
                           <div class="form-group">
                              <label>Product</label>
                              <select class="form-control" id="license-product-select" name="product_id" required="">
